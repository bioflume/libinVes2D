...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function initializeFiles(o,X,sig,Xwalls)
% initializeFiles(X,sig,eta,RS,Xwalls) does the initial writing of data
% to files and the console.  It first deletes any previous data and
% then writes the number of points X and Xwalls are the vesicles and
% solid walls
...
if o.saveData
  ...
  if o.saveData
    o.writeData(X,zeros(N,nv),0,0,0,0);
  end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function terminate = outputInfo(o,X,sigma,u,...
    Xwalls,Xtra,time,iter,dtScale,res,iflag)
% terminate = outputInfo(X,sigma,u,Xwalls,time,iter,...
%    dtScale,res,iflag) 
% computes the error in area and length and write messages 
% to the data file, the log file, and the console.  Tells 
% the simulation to stop if error in area or length is too 
% large.  X, sigma, u are the position, tension, and velocity
% of vesicles, Xwalls is the parameterization of the solid 
% walls, time is the current time, iter is the number of GMRES
% iterations, dtScale is the amount the time step is scaled by,
% res is the residual incurred from the previous time step to
% the current, and iflag is the success of gmres
...
message = ['GMRES required ' num2str(iter) ' iterations'];
o.writeMessage(message,'%s\n')
...
% function writeStress(X,output,flag) writes the stress
% to a .bin file.  flag is used to decide if it needs
% to write the locations or one of the four stress
% components
...
% function [ea,el] = errors(X) computes the errors in area
% and length of the new vesicle position
...
% plotData(X,u,Xwalls,Xtra,time,ea,el) plots 
% the current configuration with title
% X is the vesicle position, u is the vesicle velocity, Xwalls
% is the solid wall positions, Xtra is the tracer locations,
% time is the current time, ea and el are the errors in
% area and length
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function summary(o,matvecs,derivs,fmms,naccept,nreject,totTime)
