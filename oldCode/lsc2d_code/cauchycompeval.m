...
if nargout==1  % no deriv wanted... (note sum along 1-axis faster than 2-axis)
  ...
  if side=='e'
    pcomp = comp .* repmat(1./(s.x(:)-s.a), [1 M]);
  else 
    pcomp = comp; 
  end  % pcomp are weights and bary poles appearing in J0
  ...
  vc = prefac .* I0./J0;    
  % bary form (poss overall pole)

  dv = repmat(vb(:),[1 M]) - repmat(vc(:).',[N 1]); 
  % v value diff mat
  [jj ii] = ind2sub(size(invd),find(abs(invd) > 1/o.delta)); 
  % bad pairs indices
  if side=='e' % exterior:
    for l=1:numel(jj) 
      j=jj(l); 
      i=ii(l); % loop over node-targ pairs too close
      p = sum(comp(:,i).*(vb(j)./(s.x(:)-s.a)-vb(:)./(s.x(j)-s.a)))/...
        sum(pcomp(:,i)); 
        % p is v_j - (x-a)/(yj-a)*v(x) for x=i'th target
      dv(j,i) = prefac(i) * ((s.x(j)-s.a)*p + (x(i)-s.x(j))*vb(j));
    end 
    % pole-corrected for dv, gives bary stability for close node-targ pairs
  else  % interior:
