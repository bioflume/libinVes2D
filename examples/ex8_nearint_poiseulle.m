function  ex8_nearint_poiseulle() 
...
rh=zeros(N,2);
rh(:,1)=Xsou(N+1:2*N).*(1-Xsou(N+1:2*N));
% rh(:,1)=1;
% rh(:,2)=2;


% solving for density
D  = stokes_dl_singular_matrix(toim(s.x),toim(s.nx),s.sp,toim(s.tang),s.cur);
Nm  = toim2(s.nx)*(toim2(s.nx).*[s.w;s.w])';
D  = D + Nm; %rank modification to make D invertible
den = (-.5*eye(2*N)+D)\rh(:);

fcur=zeros(N,1);
fcur(:)=den(1:N)+1i*den(N+1:end);
...
for i=1:numtar
    ...
    hold on;
    ucnerr(:,i)=ucni(:,i)-Xtar(ntar+1:end,i).*(1-Xtar(ntar+1:end,i));
end
% figure;
% scatter(Xtar(1:ntar,1),Xtar(ntar+1:end,1),10.1,abs(ucni(:,1)+(1+2i)),'filled');
% figure;
% plot_log_image([1,numtar],[1,ntar],abs(ucnierr));
...
%     );
    ...
    fprintf('Values in tagrets for new code #%d: max=%g,min=%g\n',i, ...
     max(abs(ucnierr(:,i) )), min(abs(ucnierr(:,i) ))   );
...
subPath = pwd; subPath = [subPath(1:ii) ['src' filesep 'lsc2d_code']];
