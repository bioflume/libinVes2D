function Xfinal = Ves2DCol(X,Xwalls,Xrig,prams,options,Xtra,pressTar)
...
Nrd = prams.Nrd; % number of points per rigid body
nvrd = prams.nvrd; % number of rigid body  
...
if nvrd
  oc = curve;
  [~,~,lennvrd] = oc.geomProp(Xrig);
  lenMax = max([lenMax,lennvrd]);
  Nmax = Nrd;
end
prams.minSep = prams.minSep*lenMax/Nmax;
% end of set minSep
...
if nv
  if options.restart
    [posx,posy,ten,ux,uy,eta,RS,wallx,wally,ea,el,time,n,nv,nrd,nvrd,posxrd,posyrd,etard] = loadFilenew(options.restartFile);
    Xstore(:,:,1) = [posx(:,:,end);posy(:,:,end)];
    sigStore(:,:,1) = ten(:,:,end);
    uStore(:,:,1) = [ux(:,:,end);uy(:,:,end)];
    etaStore(:,:,1) = eta(:,:,end);
    RSstore(:,:,1) = RS(:,:,end); 
  else
    tt.withRigid = false;
...
while time < prams.T - 1e-10
  ...
  if options.order == 1
    pertic = tic;
    [X,sigma,u,eta,RS,iter,accept,dtScale,res,iflag,FC,Xrig,etaRig] = ...
        tt.GSSDCCOL(Xstore,sigStore,uStore,...
            etaStore,RSstore,prams.kappa,...
            prams.viscCont,walls,wallsCoarse,om,time,accept,FC,XrigStore,etaRigStore);
    pertoc = toc(pertic);
