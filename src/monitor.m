...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function initializeFiles(o,X,sig,eta,RS,Xrig,etaRig,Xwalls,Xtra,pressTar)
...
if o.saveData
  ...
  o.writeData(X,sig,X*0,eta,RS,0,0,0,0,Xrig,etaRig);
...
function terminate = outputInfo(o,X,sigma,u,eta,RS,...
    Xwalls,Xtra,time,iter,dtScale,res,iflag,Xrig,etaRig)
...
% Begin plotting
if o.usePlot
  o.plotData(X,u,Xwalls,Xtra,time,ea,el,Xrig);
  pause(0.01)
end
% End plotting
...
if o.saveData
  ...
  o.writeData(X,sigma,u,eta,RS,ea,el,time,res,Xrig,etaRig);

  if o.Nbd > 0
    %o.writeDataWithEta(X,sigma,eta,RS,ea,el,time,res,Xrig,etaRig);
  end
end
% End saving data
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function plotData(o,X,u,Xwalls,Xtra,time,ea,el,Xrig)
...
% Plot all vesicles
if ~isempty(Xrig)
  [xrig,yrig] = oc.getXY(Xrig);
  plot([xrig;xrig(1,:)],[yrig;yrig(1,:)],'b-','linewidth',1)
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function writeData(o,X,sigma,u,eta,RS,ea,el,time,res,Xrig,etaRig)
...
output = [x(:);y(:);sigma(:);ux(:);uy(:);eta(:);RS(:);Xrig(:);etaRig(:);ea;el;time];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function writeDataWithEta(o,X,sigma,eta,RS,ea,el,time,res,Xrig,etaRig)
